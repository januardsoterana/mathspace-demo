...
class NewsroomPageAU extends React.Component<Props, State> {
  scrollMarker: any;
  ...
  render() {
    const featuredCard = getFeaturedNews(this.props.newsCards, 'AU')[0];
    const newsCardList = getNonFeaturedNews(this.props.newsCards, 'AU');
    return (
      <Fragment>
        ...
        <Page title="Newsroom :: Mathspace">
          ...
          <Footer backgroundColor={colors.purpleHeart} country="AU" />
        </Page>
        <style jsx>{`
          .heroWrapper {
            position: relative;
...
NewsroomPageAU.getInitialProps = async ({ req }): Promise<Props> => {
