...
const CustomerStories = ({ banners, customerStories }: Props) => {
  const australianStories = customerStories.filter(
    story => story.country === 'AU',
  );
  const featuredStories = australianStories.filter(story => !!story.featured);
  return (
    <Fragment>
      <Page title="Customer Stories :: Mathspace">
        <Header country="AU" bannerData={banners} />
        {featuredStories[0] && (
          <Featured story={featuredStories[0]} country="AU" />
        )}
        <StoryList stories={australianStories} country="AU" />
        <CTA />
        <Footer country="AU" />
      </Page>
    </Fragment>
